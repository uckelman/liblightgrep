...
SCOPE_TEST(reduceNegativeLookarounds_LPLBWhackdRPWhadkdPLPLAWhackdRP_Test) {
  ParseTree tree;
  SCOPE_ASSERT(parse({"(?<!\\d)\\d+(?!\\d)", false, false}, tree));
  SCOPE_ASSERT(reduceNegativeLookarounds(tree));
...
SCOPE_TEST(shoveLookaroundsOutward_aLPWhackAOrLPLBPNegCCbRP) {
  ParseTree tree;
  SCOPE_ASSERT(parse({"a(\\A|(?<=[^b]))", false, false}, tree));
  SCOPE_ASSERT(shoveLookaroundsOutward(tree));
  SCOPE_ASSERT_EQUAL("[a]", unparse(tree));
}
